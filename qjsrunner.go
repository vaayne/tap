...
// runScriptQJS tries to execute the script in QuickJS with a Go-backed fetch().
// Returns the result or an error if execution fails.
func runScriptQJS(script *Script, args map[string]string) (any, error) {
	rt, err := qjs.New()
	if err != nil {
	...
	// Wrap as: await (async function(args) { ... })({...})
	js := fmt.Sprintf(`await (%s)(%s)`, script.Body, string(argsJSON))
	...
	// Convert QJS value to Go via JSON.stringify
...
// Supports: fetch(url) and fetch(url, {method, headers, body})
...
func injectFetch(ctx *qjs.Context) {
	...
	// Add .json() and .text() methods to the response via a wrapper
	// We wrap fetch to return an object where json()/text() work
